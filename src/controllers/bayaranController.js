...
// ==========================================
// 1. CIPTA BIL (DENGAN KESELAMATAN BACKEND)
// ==========================================
export const ciptaBil = async (req, res) => {
    ...
    try {
        // --- SEMAKAN TRANSAKSI PENDING (15 MINIT BLOCKER) ---
...
// ==========================================
// HELPER DALAMAN: Proses bayaran berjaya
// (kongsi logik antara webhook & semakan manual)
// ==========================================
const prosesBayaranBerjaya = async (billcode) => {
    ...
    // Aktifkan no_ahli untuk ahli MANUAL selepas FPX pertama berjaya.
    // (Biro Angkasa sudah ada no_ahli sejak awal, jadi ini hanya jana
    //  jika no_ahli masih kosong.)
    const [ahli] = await db.query(
        'SELECT no_ahli FROM users WHERE no_kp = ?', 
        [no_kp]
    );
    ...
    if (ahli.length > 0) {
        ...
        if (!noAhliSedia || String(noAhliSedia).trim() === '') {
            ...
            await db.query(
                'UPDATE users SET no_ahli = ? WHERE no_kp = ?', 
                [noAhliBaru, no_kp]
            );
        }
    }
...
// ==========================================
// 2. WEBHOOK CALLBACK (DIPANGGIL OLEH BANK)
// ==========================================
...
export const getSejarahBayaran = async (req, res) => {
    ...
    try {
        ...
        const [rows] = await db.query(`
            SELECT 
                billCode, amaun, status, keterangan, 
...
// ==========================================
// 4. SEMAKAN MANUAL API (AUTO-POLLING FRONTEND)
// ==========================================
